...
class ParkLocation {
  ...
  async createParkLocation(parkingCreator, parkLocation) {
    const { carId, userWhoParkId, time, timeLimit, location, reminder } =
      parkLocation;
    ...
    const creatorOwnsCar = carUsers.some((user) => user.user_id === parkingCreator.id);
    const parkerOwnsCar = carUsers.some((user) => user.user_id === userWhoParkId);
    ...
    const createParkLocation = new PS({
      ...
      values: [parkingId, carId, userWhoParkId, parkingCreator.id, time, timeLimit, location, reminder],
    });
...
module.exports = new ParkLocation();
